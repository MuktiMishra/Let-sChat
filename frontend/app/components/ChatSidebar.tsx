...
interface ChatSidebarProps {
  ...
  loggedInUser: User | null;
  chats: any[] | null;
  selectedUser: string | null; // ✅ keep old naming
  setSelectedUser: (userId: string | null) => void; // ✅ keep old naming
  handleLogout: () => void;
  createChat: (user: User) => void;
}
...
const ChatSidebar = ({
  ...
  return (
    <aside
      ...
      sm:translate-x-0 transition-transform duration-300 flex flex-col`} // ✅ fixed flex
    >
      {/* Header */}
      <div className="p-6 border-b border-gray-700">
        <div className="sm:hidden flex justify-end">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-600 flex items-center justify-center">
              <MessageCircle className="w-5 h-5 text-white" />
            </div>
      ...
      <div className="flex-1 overflow-hidden px-4 py-2">
        {showAllUsers ? (
          ...
          <div className="space-y-2 overflow-hidden h-full pb-4">
            {chats.map((chat) => {
              const latestMessage = chat.chat.latestMessage;
              const isSelected = selectedUser === chat.chat._id; // ✅ fixed
              const isSentByMe =
              ...
              return (
                <button
                  key={chat.chat._id}
                  onClick={() => {
                    setSelectedUser(chat.chat._id); // ✅ fixed
                    setSidebarOpen(false);
                  }}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        >
                          {chat.user?.name || "Unknown"} {/* ✅ safe */}
                        </span>

                        {unseenCount > 0 && (
                          <div className="bg-red-600 text-white text-xs font-bold rounded-full min-w-[22px] h-[22px] flex items-center justify-center px-2">
                            {unseenCount > 99 ? "99+" : unseenCount}
                          </div>
                        )}
                      </div>
      ...
      <div className="p-4 border-t border-gray-700 space-y-2">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-lg hover:bg-red-600 transition-colors text-red-500 hover:text-white"
        >
          <div className="p-1.5 bg-red-600 rounded-lg">
            <LogOut className="w-4 h-4 text-gray-300" />
          </div>
